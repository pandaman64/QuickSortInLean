...
def partitionImpl_permuted' {α : Type} [Ord α]
...
theorem partitionImpl_permuted {α : Type} [Ord α]
  {n : Nat} (arr : Vec α n) (first i j : Fin n)
  (fi : first ≤ i) (ij : i ≤ j) :
  Nonempty (permuted n first j arr (partitionImpl arr first i j fi ij).2) := by
  induction arr, first, i, j, fi, ij using partitionImpl.induct with
...
def partition_permuted' {α : Type} [Ord α]
...
def quickSortImpl_permuted {α : Type} [Ord α]
...
def quickSort'_permuted  {α : Type} [Ord α] {n : Nat} (arr : Vec α n) :
